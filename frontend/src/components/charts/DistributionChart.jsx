...
/**
 * DistributionChart — recharts histogram for trade PnL distribution.
 *
 * @param {Array}  data   - [{ range: '$0–$100', count: 12, positive: true }]
 * @param {number} height
 */
export default function DistributionChart({ data = [], height = 200 }) {
  ...
  const CustomTooltip = ({ active, payload, label }) => {
    ...
    return (
      <div style={{
        ...
        <div style={{ color: isDark ? COLORS.darkTextPrimary : '#111827', fontWeight: 600 }}>
  ...
  return (
    <ResponsiveContainer width="100%" height={height}>
      <BarChart data={data} margin={{ top: 4, right: 4, bottom: 4, left: 0 }} barSize={18}>
        <CartesianGrid
          strokeDasharray="3 3"
        ...
        <Bar dataKey="count" radius={[3, 3, 0, 0]}>
          {data.map((entry, idx) => (
            <Cell
              key={idx}
              fill={entry.positive ? COLORS.pnlGreen : COLORS.pnlRed}
              fillOpacity={0.75}
            />
          ))}
        </Bar>

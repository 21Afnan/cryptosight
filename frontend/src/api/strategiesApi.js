...
export async function getStrategyById(id) {
  try {
    ...
    console.warn(`Failed to fetch strategy details for ${id}:`, err.message);
  }

  // Fallback to strategies list if direct endpoint ID fails
  const { data } = await getStrategies({ pageSize: 500 });
  const strategy = data.find((s) => String(s.strategy_id) === String(id) || s.strategy_name === String(id));
  return strategy || { strategy_id: id, strategy_name: String(id), exchange: 'bybit', symbol: 'BTCUSDT', target_timeframe: '1h' };
}
